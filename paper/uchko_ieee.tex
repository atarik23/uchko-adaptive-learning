...
\title{Uchko: An Adaptive Mathematics Learning Platform with Predictive Modeling and Knowledge Tracing}
...
\begin{abstract}
Adaptive learning systems must connect predictive models with persistent learner state, curriculum-aligned content, and a usable delivery interface. This paper presents Uchko, a web platform for adaptive middle-school mathematics practice. The deployed hybrid combines an XGBoost classifier, which estimates the probability of an independently successful response, with Bayesian Knowledge Tracing (BKT), which maintains an interpretable mastery probability for each skill. A deterministic policy favors weak skills and problems whose predicted success is near a 0.65 target. All historical features use only prior interactions, and train, validation, and test partitions are disjoint at the student level. After filtering and metadata joins, the modeling data contain 1,693,947 interactions from 5,000 students. On 253,718 matched next-response test cases, XGBoost achieves 0.8125 ROC AUC, 0.7443 accuracy, and 0.5058 log loss. A compact gated recurrent Deep Knowledge Tracing (DKT) model reaches 0.7620 ROC AUC and 0.5542 log loss, outperforming a history baseline but not XGBoost. This controlled negative result supports retaining the more accurate and operationally simpler boosted-tree predictor. The deployed XGBoost--BKT hybrid is integrated into a Django application with role-based student and professor views, persistent attempts and sessions, curriculum exploration, sanitized imported content, and explicit separation between answer correctness and unaided success. Uchko demonstrates how comparative educational data mining experiments can inform an end-to-end adaptive learning implementation.
\end{abstract}

\begin{IEEEkeywords}
adaptive learning, educational data mining, knowledge tracing, Deep Knowledge Tracing, XGBoost, student modeling
\end{IEEEkeywords}
...
\begin{itemize}
    ...
    \item a controlled comparison between aggregate history, recurrent DKT, BKT, and gradient-boosted prediction under a common student-disjoint split;
...
Within knowledge tracing, BKT remains influential because its parameters have explicit cognitive interpretations, while DKT and later attentive models capture richer dependencies in student sequences \cite{corbett1994knowledge,piech2015dkt,ghosh2020akt}. Surveys emphasize that comparisons are sensitive to preprocessing, splitting, and model configuration \cite{abdelrahman2023survey}. Uchko therefore prioritizes a reproducible, student-disjoint evaluation and includes a controlled DKT experiment rather than assuming that a deep sequence model must be superior. The distinctive focus is not a novel knowledge-tracing architecture, but evidence-based selection and integration of complementary components into a tested application whose instructional policy is inspectable.
...
The comparative DKT model follows the recurrent formulation of Piech et al. \cite{piech2015dkt}. Each interaction is encoded as a joint skill-response token and mapped to a 64-dimensional embedding. A single gated recurrent unit with 128 hidden units processes each student's ordered history; a linear output layer predicts correctness for the next observed skill. Sequences are divided into chunks of at most 100 prediction steps with a one-event boundary overlap. The model contains 113,881 parameters and is optimized on CPU with Adam, learning rate $10^{-3}$, batch size 128, dropout 0.15, weight decay $10^{-5}$, and gradient clipping at 5. Training permits 15 epochs with patience three according to validation log loss. The best checkpoint occurs at epoch 15. DKT uses the same student partitions and target as the other predictors; fair model comparison excludes the first response of each test student, for which no prior sequence exists.
...
\caption{Uchko's hybrid adaptive loop. XGBoost predicts candidate success, BKT tracks skill mastery, and a deterministic policy converts both outputs into the next activity; the resulting response is persisted as new history.}
...
The implementation redesigns an earlier single-user Streamlit proof of concept as a persistent multi-user Django system, separating domain services, authorization, storage, and server-rendered presentation. Django authentication stores salted password hashes, CSRF middleware protects state-changing forms, and role- and ownership-aware checks separate student records from professor views. These measures make Uchko a production-oriented research prototype, although use with real learners would still require a dedicated security and privacy review.

At inference time, the application reconstructs the same features used during training from a student's persisted history. The serialized preprocessor and XGBoost model run on CPU even though model fitting used GPU acceleration. Candidate items must have a supported answer and belong to an active course. After submission, answer checking supports tolerance-aware numeric responses and single-choice alternatives. DKT is retained as a reproducible comparison but is not deployed because it is less accurate than XGBoost and would add stateful sequence inference without an empirical benefit.
...
\centering
\includegraphics[width=\linewidth]{figures/uchko_practice.png}\\[-1mm]
\small (a) Student adaptive-practice view
\end{minipage}\hfill
\begin{minipage}[t]{0.485\textwidth}
\centering
\includegraphics[width=\linewidth]{figures/uchko_professor.png}\\[-1mm]
\small (b) Professor course dashboard
\end{minipage}
\caption{Role-aware Uchko interfaces. Students receive a focused next activity and progress signal, while professors see enrollment-level activity and can open individual progress profiles. The displayed population is synthetic demonstration data.}
\label{fig:interface}
\end{figure*}

\section{Results}
Table~\ref{tab:results} reports performance on the 253,718 test cases shared by the history, DKT, and XGBoost evaluations. The first response from each student is omitted because DKT requires a preceding interaction. The global validation baseline has an AUC of 0.5 by construction and is omitted for compactness. The linear model reaches 0.7868 AUC and 0.5393 log loss on validation, while validation results for XGBoost are 0.8087 and 0.5113. This advantage motivated selection of XGBoost before final testing.

\begin{table}[t]
\centering
\caption{Matched performance on 750 held-out test students}
\label{tab:results}
...
\midrule
History & 0.6932 & 0.6732 & 0.7602 & 0.6073 & 0.2097 \\
DKT & 0.7620 & 0.7167 & 0.7878 & 0.5542 & 0.1872 \\
XGBoost & \textbf{0.8125} & \textbf{0.7443} & \textbf{0.8060} & \textbf{0.5058} & \textbf{0.1692} \\
\bottomrule
\end{tabular}
\end{table}

DKT improves AUC by 0.0688 over matched history and reduces log loss by 8.7\%, demonstrating that ordered skill-response sequences contain useful predictive structure. However, XGBoost exceeds DKT by 0.0505 AUC and reduces its log loss by a further 8.7\%. XGBoost improves AUC by 0.1193 over matched history and lowers the Brier score by 19.3\%. Its matched test AUC of 0.8125 is close to the original all-row test estimate of 0.8122, indicating that exclusion of each student's first response does not materially change the conclusion.

BKT, evaluated sequentially on the full held-out test partition, obtains 0.6850 AUC, 0.6772 accuracy, 0.7671 F1, 0.6105 log loss, and 0.2106 Brier score. It does not outperform the history baseline as a next-response predictor. This result should not be hidden: BKT's contribution is instead an interpretable state transition that can be updated after every response and inspected per skill. The deployed hybrid assigns prediction and state estimation to the models that best support each role.
...
\section{Discussion}
The experiment supports four conclusions. First, prior learner history and problem metadata together contain considerably more predictive information than aggregate history alone. XGBoost benefits from non-linear interactions among sparse item categories, temporal signals, hint histories, and skill-specific performance. Second, DKT's gain over history confirms that a recurrent representation of ordered skill-response events is useful, but deep learning is not automatically the strongest choice: the feature-rich boosted model performs better on every reported metric. Third, student-disjoint partitions offer evidence that the models generalize to new learner accounts rather than memorizing students seen during training. Fourth, deployment requires design decisions that offline metrics do not resolve. The target probability, recent-item exclusion window, weakest-skill set size, and handling of assistance are transparent policy choices that require eventual user-study validation.
...
The DKT result is informative despite not becoming part of the deployed path. It was trained on the same student-disjoint split and evaluated against XGBoost on exactly the same next-response rows, eliminating a common source of misleading comparison. Its lower AUC and higher probabilistic losses do not show that recurrent knowledge tracing is generally ineffective; they show that this compact configuration and interaction representation do not outperform Uchko's engineered prior-history and content features. Larger architectures, longer optimization, and attentive knowledge tracing remain possible extensions, but would require new validation rather than deployment by assumption.

\section{Conclusion}
Uchko demonstrates an end-to-end path from protected educational interaction data to adaptive web-based practice. On unseen students, XGBoost substantially outperforms aggregate history and a compact recurrent DKT model for next-response prediction, while BKT provides a simple and inspectable per-skill state. The negative DKT comparison supports retaining the more accurate and simpler deployed predictor instead of selecting a model by architectural novelty. A deterministic policy combines prediction and mastery to focus on weak skills at a controlled level of challenge. Persistent data models, role-based authorization, safe content rendering, and explicit treatment of assisted answers turn the experiment into a functioning platform rather than an isolated notebook. The remaining research question is educational impact: future work should test whether the policy improves learning and whether its recommendations remain fair, understandable, and useful in authentic classrooms.
